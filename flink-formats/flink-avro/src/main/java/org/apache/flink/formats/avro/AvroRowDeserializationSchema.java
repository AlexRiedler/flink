...
public class AvroRowDeserializationSchema extends AbstractDeserializationSchema<Row> {
		...
		switch (schema.getType()) {
			...
			case LONG:
				if (info == Types.SQL_TIMESTAMP) {
					...
					return Instant.EPOCH.plus((long) object, ChronoUnit.MICROS);
				}
				return object;
			case FLOAT:
			case DOUBLE:
	...
	private Date convertToDate(Object object) {
		...
		if (object instanceof Integer) {
			...
			millis = Date.from(value.atStartOfDay(ZoneId.systemDefault()).toInstant()).getTime(); // value.toDate().getTime();
		}
